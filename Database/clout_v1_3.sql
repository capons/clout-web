...
--
-- Хост: localhost
-- Время создания: Ноя 17 2016 г., 00:27
...
-- Функции
--
CREATE DEFINER=`root`@`localhost` FUNCTION `active` (`user_id` INT) RETURNS TEXT CHARSET utf8 RETURN (SELECT case when (SELECT clout_v1_3cron.store_schedule.reservation_status FROM clout_v1_3cron.store_schedule where clout_v1_3cron.store_schedule._user_id = user_id GROUP BY clout_v1_3cron.store_schedule._user_id) IS NULL 
...
CREATE DEFINER=`root`@`localhost` FUNCTION `been_helped` (`user_id` INT) RETURNS TEXT CHARSET utf8 RETURN (
SELECT
 IFNULL( ( SELECT 
 	 		(CASE 
				        WHEN s.status = 'active' AND s.reservation_status = 'confirmed' THEN 'Been Helped'
				        ELSE 'not helped'
   		END)
  	FROM clout_v1_3.store_schedule s
  	LEFT JOIN clout_v1_3cron.promotions p ON p.id = s._promotion_id 
 	WHERE s._user_id = user_id AND s.reservation_status='confirmed' AND s.status = 'active' AND p.promotion_type='perk' 

),'not helped' ) as been_helped_status
)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `category_spending` (`user_id` INT(255)) RETURNS INT(11) RETURN(SELECT sum(clout_v1_3cron.cacheview__store_score_by_store.my_direct_competitors_spending_lifetime) FROM clout_v1_3cron.cacheview__store_score_by_store WHERE clout_v1_3cron.cacheview__store_score_by_store.user_id =  user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `competitor_spending` (`user_id` INT) RETURNS INT(11) RETURN(SELECT sum(clout_v1_3cron.cacheview__store_score_by_store.my_direct_competitors_spending_lifetime) FROM clout_v1_3cron.cacheview__store_score_by_store WHERE clout_v1_3cron.cacheview__store_score_by_store.user_id =  user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `get_distance` (`t` INT(50), `h` INT(50), `k` INT(50), `hh` INT(50)) RETURNS INT(50) NO SQL
return 10$$

CREATE DEFINER=`root`@`localhost` FUNCTION `get_upcoming_date` (`user_id` INT(255), `lati` FLOAT(5), `langi` FLOAT(5)) RETURNS DATETIME RETURN (SELECT upcoming_date_.schedule_date FROM (SELECT upcoming_date.schedule_date FROM (SELECT MIN(distance_count.distance), distance_count.schedule_date
...
CREATE DEFINER=`root`@`localhost` FUNCTION `here_now_clock` (`user_id` INT) RETURNS VARCHAR(50) CHARSET latin1 RETURN (SELECT (case when (SELECT clout_v1_3.user_geo_tracking.source FROM clout_v1_3.user_geo_tracking
...
CREATE DEFINER=`root`@`localhost` FUNCTION `in_store_spending` (`user_id` INT) RETURNS INT(11) RETURN(SELECT sum(clout_v1_3cron.cacheview__store_score_by_store.my_store_spending_lifetime) FROM clout_v1_3cron.cacheview__store_score_by_store WHERE clout_v1_3cron.cacheview__store_score_by_store.user_id =  user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `last_activity_ed` (`user_id` INT) RETURNS DATETIME RETURN (SELECT max(clout_v1_3.user_geo_tracking.tracking_time) FROM `user_geo_tracking` WHERE  clout_v1_3.user_geo_tracking._user_id = user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `other_reser_ed` (`user_id` INT(255)) RETURNS INT(11) RETURN (SELECT count(*) FROM clout_v1_3.store_schedule  where clout_v1_3.store_schedule._user_id<>user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `priority` (`user_id` INT) RETURNS INT(11) RETURN (SELECT IFNULL( (SELECT
...
CREATE DEFINER=`root`@`localhost` FUNCTION `promo_spendings` (`store_id` INT) RETURNS INT(50) RETURN ((SELECT sum(clout_v1_3cron.promotions.amount) FROM clout_v1_3cron.promotions where clout_v1_3cron.promotions.owner_id = store_id and clout_v1_3cron.promotions.owner_type = 'person'))$$

CREATE DEFINER=`root`@`localhost` FUNCTION `related_spending` (`user_id` INT) RETURNS INT(11) RETURN(SELECT sum(clout_v1_3cron.cacheview__store_score_by_store.related_categories_spending_lifetime) FROM clout_v1_3cron.cacheview__store_score_by_store WHERE clout_v1_3cron.cacheview__store_score_by_store.user_id =  user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `reservation_clock` (`user_id` INT) RETURNS VARCHAR(50) CHARSET latin1 RETURN (SELECT (case when (SELECT clout_v1_3.store_schedule._user_id FROM clout_v1_3.store_schedule WHERE clout_v1_3.store_schedule._user_id = user_id GROUP BY clout_v1_3.store_schedule._user_id) IS NULL
           
            then 'No reservation'
            else (case when (SELECT clout_v1_3.store_schedule._user_id FROM clout_v1_3.store_schedule WHERE clout_v1_3.store_schedule._user_id = user_id and clout_v1_3.store_schedule.reservation_status <>"confirmed" GROUP BY clout_v1_3.store_schedule._user_id) IS NULL
            		then ((SELECT  CONCAT(clout_v1_3.store_schedule.date_entere, '|', 'confirm')   FROM clout_v1_3.store_schedule WHERE clout_v1_3.store_schedule._user_id = user_id GROUP BY clout_v1_3.store_schedule._user_id))
            		else ((SELECT  CONCAT(clout_v1_3.store_schedule.date_entere, '|', 'not_confirm')   FROM clout_v1_3.store_schedule WHERE clout_v1_3.store_schedule._user_id = user_id GROUP BY clout_v1_3.store_schedule._user_id))
            		end)
            end) as reservations_clock)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `score` (`user_id` INT) RETURNS INT(11) RETURN(SELECT sum(clout_v1_3cron.cacheview__store_score_by_store.total_score) FROM clout_v1_3cron.cacheview__store_score_by_store WHERE clout_v1_3cron.cacheview__store_score_by_store.user_id =  user_id)$$

CREATE DEFINER=`root`@`localhost` FUNCTION `store_distance` (`lati` FLOAT(5), `langi` FLOAT(5)) RETURNS INT(11) RETURN (SELECT min(distance)
...
CREATE DEFINER=`root`@`localhost` FUNCTION `store_last_transaction_ed` (`user_id` INT) RETURNS DATETIME RETURN (SELECT max(clout_v1_3cron.transactions.start_date) FROM clout_v1_3cron.transactions WHERE clout_v1_3cron.transactions._user_id=user_id)$$
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__activity_log` AFTER INSERT ON `activity_log` FOR EACH ROW BEGIN
...
CREATE TABLE `cacheview__default_search_suggestions` (
...
CREATE TABLE `cacheview__search_tracking_by_user_summary` (
...
CREATE TABLE `cacheview__search_tracking_summary` (
  `search_type` varchar(100) NOT NULL,
  `user_phrase` varchar(250) NOT NULL,
  `frequency` bigint(21) DEFAULT NULL,
  `date_of_search` datetime NOT NULL,
...
CREATE TABLE `categories_level_1` (
  `id` bigint(20) NOT NULL,
...
(11, 'Total Spendings', '', 'blue_education_icon.png', 1, 'Y');
...
CREATE TABLE `categories_level_2` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `categories_level_2_naics_mapping` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `categories_level_2_naics_mapping_new` (
  `id` bigint(20) NOT NULL,
  `_category_level_2_id` bigint(20) NOT NULL,
  `naics_code` varchar(10) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `categories_level_2_sic_mapping` (
  `id` bigint(20) NOT NULL,
  `_category_level_2_id` bigint(20) NOT NULL,
...
CREATE TABLE `categories_level_2_sic_mapping_new` (
  `id` bigint(20) NOT NULL,
  `_category_level_2_id` bigint(20) NOT NULL,
  `sic_code` varchar(10) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `categories_level_2_split_JC` (
  `id` bigint(20) NOT NULL DEFAULT '0',
...
CREATE TABLE `categories_level_2_suggestions` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `categories_SPLIT_JC` (
  `id` bigint(20) NOT NULL DEFAULT '0',
...
CREATE TABLE `commissions_status_track` (
  `id` bigint(20) NOT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__contact_addresses` AFTER INSERT ON `contact_addresses` FOR EACH ROW BEGIN
...
CREATE TABLE `contact_subscribe_list` (
  `id` bigint(20) NOT NULL,
...
--
-- Структура таблицы `customer_tags`
--
...
--
-- Структура таблицы `customer_tags_group`
--

CREATE TABLE `customer_tags_group` (
  `id` int(11) NOT NULL,
  `customer_tags_id` int(11) NOT NULL,
...
--
-- Структура таблицы `default_category`
--
...
--
-- Дамп данных таблицы `default_category`
--

INSERT INTO `default_category` (`id`, `name`, `description`) VALUES
(1, 'total_spendings', NULL);
...
CREATE TABLE `plaid_categories_web` (
...
CREATE TABLE `promotions_categories_levels` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `promotions_custom_categories` (
  `id` bigint(20) NOT NULL,
  `category_id` bigint(20) DEFAULT NULL,
...
(38, NULL, NULL, 4, 'category', 'Total Spendings', 'active', 56),
(39, 1, NULL, 4, 'category', 'Arts & Entertainment', 'active', 56),
(40, 2, NULL, 4, 'category', 'Automotive', 'active', 56),
(41, 4, NULL, 4, 'category', 'Clothing & Accessories', 'active', 56);
...
CREATE TABLE `promotions_custom_levels` (
  `id` int(11) NOT NULL,
...
(42, 'Gold', 5, 0, 'active', 56, 4, 0),
(43, 'Silver', 5, 0, 'active', 56, 4, 0),
(44, 'Bronze', 5, 0, 'active', 56, 4, 0),
(45, 'Platinum', 5, 0, 'deleted', 56, 4, 0);

-- --------------------------------------------------------

--
-- Структура таблицы `promotion_default_category_level`
--

CREATE TABLE `promotion_default_category_level` (
  `id` bigint(20) NOT NULL,
  `name` varchar(50) NOT NULL DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Дамп данных таблицы `promotion_default_category_level`
--

INSERT INTO `promotion_default_category_level` (`id`, `name`) VALUES
(1, 'Gold'),
(2, 'Silver'),
(3, 'Bronze');
...
(0, 'get_promotion_default_category', 'SELECT clout_v1_3.categories_level_1.id FROM categories_level_1 WHERE categories_level_1.name = \'_NAME_\''),
...
(356, 'get_promotion_list', 'SELECT\r\n    id,\r\n    owner_id,\r\n    owner_type,\r\n    promotion_type,\r\n    IF (start_score < end_score, start_score, end_score) AS score,\r\n    number_viewed,\r\n    number_redeemed,\r\n    new_customers,\r\n    gross_sales,\r\n    is_boosted,\r\n    boost_budget,\r\n    boost_start_date,\r\n    boost_end_date,\r\n    boost_remaining,\r\n    name,\r\n    amount,\r\n    description,\r\n    status,\r\n    start_date,\r\n    end_date,\r\n    date_entered,\r\n    _entered_by,\r\n    last_updated,\r\n    _last_updated_by,\r\n    cash_back_percentage\r\n FROM clout_v1_3cron.promotions WHERE owner_id=\'_OWNER_ID_\' AND _entered_by=\'_ENTERED_BY_\' AND owner_type=\'_OWNER_TYPE_\' \r\n GROUP BY id\r\n HAVING score>=_MIN_SCORE_'),
(357, 'update_promotion_status', 'UPDATE clout_v1_3cron.promotions SET status=\'_STATUS_\' WHERE _entered_by=\'_OWNER_ID_\' AND owner_type=\'_OWNER_TYPE_\' AND id=\'_PROMOTION_ID_\''),
...
(376, 'get_promotions_by_level', 'SELECT promotions.*\nFROM clout_v1_3cron.promotions as promotions\nLEFT JOIN clout_v1_3.stores as store ON store.id = promotions.owner_id\nLEFT JOIN store_sub_categories AS store_categories ON store_categories._store_id=store.id\nLEFT JOIN promotions_custom_categories AS custom_categories ON\n	custom_categories.category_id=store_categories._category_id AND\n		(custom_categories.sub_category_id=store_categories._sub_category_id OR\n		(custom_categories.sub_category_id IS NULL AND store_categories._sub_category_id IS NULL))\nLEFT JOIN promotions_categories_levels AS connections ON connections.category_id=custom_categories.id\nWHERE store.id IS NOT NULL AND custom_categories.id IS NOT NULL\nAND connections.level_id=\'_LEVEL_ID_\' AND promotions.owner_type=\'_OWNER_TYPE_\' AND promotions.owner_id = \'_OWNER_ID_\' AND promotions._entered_by=\'_ENTERED_BY_\' AND promotions.amount>=connections.amount\nGROUP BY promotions.id'),
...
(388, 'add_promotion', 'INSERT INTO clout_v1_3cron.promotions (promotion_type, owner_id, owner_type, start_date, end_date, start_score, end_score, amount, _entered_by, name, description, cash_back_percentage) VALUES (\'_PROMOTION_TYPE_\', \'_OWNER_ID_\', \'_OWNER_TYPE_\', \'_START_DATE_\', \'_END_DATE_\',  \'_START_SCORE_\', \'_END_SCORE_\', \'_AMOUNT_\', \'_ENTERED_BY_\', \'_NAME_\', \'_DESCRIPTION_\',\'_CASH_BACK_PERCENTAGE_\')'),
(389, 'get_promotion_by_id', 'SELECT  id,\r\n          owner_id,\r\n          owner_type,\r\n          promotion_type,\r\n          IF (start_score < end_score, start_score, end_score) AS score,\r\n          number_viewed,\r\n          number_redeemed,\r\n          new_customers,\r\n          gross_sales,\r\n          is_boosted,\r\n          boost_budget,\r\n          boost_start_date,\r\n          boost_end_date,\r\n          boost_remaining,\r\n          name,\r\n          amount,\r\n          description,\r\n          status,\r\n          start_date,\r\n          end_date,\r\n          date_entered,\r\n          _entered_by,\r\n          last_updated,\r\n          _last_updated_by,\r\n          cash_back_percentage FROM clout_v1_3cron.promotions WHERE id=\'_ID_\''),
...
(407, 'get_default_level', 'SELECT * FROM `promotion_default_category_level`'),
(408, 'add_promotion_blackout_schedule', 'INSERT INTO clout_v1_3cron.promotions_display_blackout(`promotion_id`, `week_day`, `display_date`, `start_time`, `end_time`) VALUES (\'_PROMOTION_ID_\',\'_WEEK_DAY_\',\'_DISPLAY_DATE_\',\'_START_TIME_\',\'_END_TIME_\')'),
(901, 'add_promotion_rule', 'INSERT INTO clout_v1_3cron.promotion_rules(`rule_type`, `rule_details`, `_promotion_id`, `rule_amount`) VALUES (\'_RULE_TYPE_\',\'_RULE_DETAILS_\',\'_PROMOTION_ID_\',\'_TYPE_\')'),
(902, 'get_promotion_blackout', 'SELECT blackout.id,blackout.week_day,blackout.display_date,blackout.start_time,blackout.end_time FROM clout_v1_3cron.promotions_display_blackout blackout WHERE promotion_id = \'_PROMOTION_ID_\''),
(903, 'delete_blackout', 'DELETE FROM clout_v1_3cron.promotions_display_blackout WHERE id = \'_ID_\''),
(904, 'delete_promotion_rules', 'DELETE FROM clout_v1_3cron.promotion_rules WHERE _promotion_id = \'_ID_\''),
(905, 'delete_promotion', 'DELETE t1, t2,t3 FROM\r\n  clout_v1_3cron.promotions as t1\r\n  LEFT JOIN  clout_v1_3cron.promotions_display_blackout as t2 on t1.id = t2.promotion_id\r\n  LEFT JOIN  clout_v1_3cron.promotion_rules as t3 on t1.id = t3._promotion_id\r\n  WHERE  t1.id= \'_ID_\' '),
(906, 'update_schedule_reservation_status', 'UPDATE store_schedule SET reservation_status=\'_RESERV_STATUS_\', last_updated=NOW(), _last_updated_by=\'_USER_ID_\' WHERE id =\n(SELECT * FROM(SELECT id FROM clout_v1_3.store_schedule WHERE  _promotion_id=\'_ID_\' AND reservation_status=\'active\' AND status=\'active\') as t)'),
(907, 'update_perk_status', 'UPDATE clout_v1_3cron.promotions SET status=\'_STATUS_\', last_updated=\'_TIME_\', _last_updated_by=\'_UPDATE_BY_USER_ID_\' WHERE id =\r\n(SELECT * FROM(SELECT id FROM clout_v1_3cron.promotions WHERE id=\'_ID_\' AND status=\'pending\' AND promotion_type=\'perk\' ) as t)');
INSERT INTO `queries` (`id`, `code`, `details`) VALUES
(908, 'select_all_perk', 'SELECT P.id FROM clout_v1_3cron.promotions P  WHERE _entered_by = \'_USER_ID_\' AND status = \'pending\' AND promotion_type = \'perk\''),
(909, 'add_customers_tag', 'INSERT INTO `customer_tags`(`label`, `owner_id`,`is_delete`) VALUES (\'_LABEL_\',\'_OWNER_ID_\',\'_DELETE_\')'),
(910, 'add_customers_tag_group', 'INSERT INTO `customer_tags_group`(`customer_tags_id`, `user_id`) VALUES (\'_TAG_ID_\',\'_USER_ID_\')'),
(911, 'duplicate_customers_tag', 'SELECT id FROM `clout_v1_3`.`customer_tags` WHERE label = \'_LABEL_\' AND owner_id = \'_OWNER_ID_\''),
(912, 'delete_customers_tag', 'DELETE FROM `clout_v1_3`.`customer_tags` WHERE owner_id = \'_OWNER_ID_\' and id = \'_TAG_ID_\' and is_delete = \'Y\''),
(913, 'get_all_customer_tag', 'SELECT c.id, c.label FROM `clout_v1_3`.`customer_tags` c WHERE c.owner_id = \'_OWNER_ID_\''),
(914, 'get_customer_tag_by_label', 'SELECT cg.user_id FROM  `clout_v1_3`.`customer_tags_group` cg\r\n																							INNER JOIN `clout_v1_3`.`customer_tags` c  ON cg.customer_tags_id = c.id\r\n																							WHERE owner_id = \'_OWNER_ID_\' and label = \'_LABEL_\''),
(915, 'user_promotion_qualifies', 'SELECT c.user_id as user_id, IF ((SELECT sum(total_score) FROM clout_v1_3cron.cacheview__store_score_by_store cs WHERE cs.user_id=\'_USER_ID_\' AND cs.store_id=\'_STORE_ID_\' ) >= (SELECT sum(p.start_score) as score FROM clout_v1_3cron.promotions p WHERE p.id = \'_PROM_ID_\'), \'qualify\', \'disqualify\') AS qualifies\r\n FROM clout_v1_3cron.cacheview__store_score_by_store c\r\n WHERE  c.store_id = \'_STORE_ID_\'\r\n GROUP BY c.user_id');
...
--
DELIMITER $$
CREATE TRIGGER `triggerdelete__referrals` AFTER DELETE ON `referrals` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `triggerinsert__referrals` AFTER INSERT ON `referrals` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__reviews` AFTER INSERT ON `reviews` FOR EACH ROW BEGIN
...
CREATE TABLE `stores_chain_zero_temp_jc` (
  `id` bigint(20) NOT NULL DEFAULT '0',
  `_chain_id` bigint(20) NOT NULL,
  `name` varchar(500) NOT NULL,
  `clout_id` varchar(100) NOT NULL,
...
CREATE TABLE `stores_dba_TEMP_JC` (
...
CREATE TABLE `stores_trans_cat_TEMP_JC` (
  `id` bigint(20) NOT NULL DEFAULT '0',
  `name` varchar(500) NOT NULL,
  `longitude` varchar(10) NOT NULL,
  `latitude` varchar(10) NOT NULL,
...
CREATE TABLE `stores_trans_cat_TEMP_JC2` (
  `id` bigint(20) NOT NULL DEFAULT '0',
  `name` varchar(500) NOT NULL,
  `longitude` varchar(10) NOT NULL,
  `latitude` varchar(10) NOT NULL,
...
CREATE TABLE `store_competitors_dev` (
  `id` bigint(20) NOT NULL,
  `_store_id` bigint(20) DEFAULT NULL,
  `store_name` varchar(500) NOT NULL,
  `competitor_id` bigint(20) NOT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__store_favorites` AFTER INSERT ON `store_favorites` FOR EACH ROW BEGIN
...
CREATE TABLE `store_offer_requests` (
  `id` bigint(20) NOT NULL,
  `_user_id` bigint(20) NOT NULL,
  `_store_id` bigint(20) NOT NULL,
...
CREATE TABLE `store_owner_stores` (
  `id` bigint(20) NOT NULL,
  `_store_owner_id` bigint(20) NOT NULL,
  `_store_id` bigint(20) NOT NULL,
  `date_entered` datetime NOT NULL,
...
CREATE TABLE `store_products_or_services` (
  `id` bigint(20) NOT NULL,
  `_store_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `store_products_or_services_new` (
  `id` bigint(20) NOT NULL,
  `_store_id` bigint(20) DEFAULT NULL,
  `name` varchar(200) NOT NULL,
  `no_of_purchases` bigint(20) NOT NULL
...
CREATE TABLE `store_sales_channels` (
  `id` bigint(20) NOT NULL,
  `_store_id` bigint(20) DEFAULT NULL,
...
(1, 12, 189, 12, 'sdfsdf', 'boggob@ram.ru', 234534545, 345345, 'Mobile', '2016-08-10 04:39:24', 0, 'ertertret', 'N', 'erter ertert ert', 'N', 'N', 'N', 'confirmed', 'active', '2016-08-30 00:00:00', NULL, '2016-08-10 07:10:19', NULL);
...
CREATE TABLE `store_sub_categories_new` (
  `id` bigint(20) NOT NULL,
  `_store_id` bigint(20) DEFAULT NULL,
  `_category_id` bigint(20) DEFAULT NULL,
  `_sub_category_id` bigint(20) DEFAULT NULL
...
CREATE TABLE `store_sub_categories_new_tst` (
  `id` bigint(20) NOT NULL DEFAULT '0',
  `_store_id` bigint(20) DEFAULT NULL,
  `_category_id` bigint(20) DEFAULT NULL,
  `_sub_category_id` bigint(20) DEFAULT NULL
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__survey_responses` AFTER INSERT ON `survey_responses` FOR EACH ROW BEGIN
...
CREATE TABLE `transactions_raw_TEMP_JC` (
  `id` bigint(20) NOT NULL,
...
(54, '1471589968-54', 'ASSADAS', '', 'QWDQWDQWD', 'male', '1987-12-29', 'bog@ram.ru', '3453453453', 'mobile', 'N', 'N', '', '', 'N', '', 'N', '', '', '', '', 0, '', '23453', 'N', 'N', 'N', 'N', 'N', 'N', 0, 0, 0, 0, '', 'unknown', '', 'pending', 0, 'N', 'N', 'N', '2016-08-18 23:59:20', 54, '2016-08-18 23:59:23', 54),
...
(56, '1471601127-56', 'ertertret', '', 'ertertert', 'female', '1990-11-18', 'merch@ram.ru', '34534534535', 'mobile', 'Y', 'N', '', '', 'N', '', 'N', '', '', '', '', 0, '', '35345', 'N', 'N', 'N', 'N', 'N', 'N', 0, 0, 0, 0, '', 'unknown', '', 'active', 0, 'N', 'N', 'N', '2016-08-19 03:05:22', 56, '2016-08-19 03:05:24', 56),
...
(59, '1471601673-59', 'ertertert', '', 'ertertertert', 'female', '1987-12-16', 'boggo@ram.ru', '43534534355', 'mobile', 'Y', 'N', '', '', 'N', '', 'N', '', '', '', '', 0, '', '34534', 'N', 'N', 'N', 'N', 'N', 'N', 0, 0, 0, 0, '', 'unknown', '', 'active', 0, 'N', 'N', 'N', '2016-08-19 03:14:28', 59, '2016-08-19 03:14:29', 59),
...
--
DELIMITER $$
CREATE TRIGGER `triggerdelete__users` AFTER DELETE ON `users` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `triggerinsert__users` AFTER INSERT ON `users` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `triggerupdate__users` AFTER UPDATE ON `users` FOR EACH ROW BEGIN
...
CREATE TABLE `user_facebook_data` (
  `id` bigint(20) NOT NULL,
...
(1, 12, '2015-08-10 10:31:49', '-98.18303', '26.25544', '', 'Los Angeles', '90036', 'California', 1, 13, '', 'checkin'),
(2, 2, '2015-07-22 10:34:22', '-118.3502', '34.069', '', 'Los Angeles', '90036', 'California', 4213014, 13, '', 'checkin'),
(3, 2, '2015-07-22 10:35:08', '-108.3502', '34.069', '', 'Los Angeles', '90036', 'California', 4213014, 13, '', 'checkin'),
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__user_geo_tracking` AFTER INSERT ON `user_geo_tracking` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__user_locations` AFTER INSERT ON `user_locations` FOR EACH ROW BEGIN
...
CREATE TABLE `user_search_learned_phrases` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `user_search_tracking` (
  `id` bigint(20) NOT NULL,
  `_user_id` bigint(20) DEFAULT NULL,
  `search_type` varchar(100) NOT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__user_search_tracking` AFTER INSERT ON `user_search_tracking` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__user_security_settings` AFTER INSERT ON `user_security_settings` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `triggerupdate__user_security_settings` AFTER UPDATE ON `user_security_settings` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `triggerinsert__user_social_media` AFTER INSERT ON `user_social_media` FOR EACH ROW BEGIN
...
--
CREATE TABLE `view__default_search_suggestions` (
`user_id` varchar(100)
...
--
CREATE TABLE `view__promotions_summary` (
`id` bigint(20)
,`owner_id` bigint(20)
...
--
-- Структура таблицы `view__search_tracking_summary`
--

CREATE TABLE `view__search_tracking_summary` (
  `search_type` varchar(100) DEFAULT NULL,
  `user_phrase` varchar(300) DEFAULT NULL,
  `frequency` bigint(21) DEFAULT NULL,
  `date_entered` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
-- Структура таблицы `view__user_details`
--

CREATE TABLE `view__user_details` (
  `user_id` bigint(20) DEFAULT NULL,
  `first_name` varchar(300) DEFAULT NULL,
  `last_name` varchar(300) DEFAULT NULL,
  `email_address` varchar(300) DEFAULT NULL,
  `email_verified` enum('Y','N') DEFAULT NULL,
  `mobile` varchar(20) DEFAULT NULL,
  `mobile_verified` enum('Y','N') DEFAULT NULL,
  ...
  `driver_license` varchar(300) DEFAULT NULL,
  `driver_license_verified` enum('Y','N') DEFAULT NULL,
  `ssn` varchar(100) DEFAULT NULL,
  `address_verified` enum('Y','N') DEFAULT NULL,
  `date_joined` datetime DEFAULT NULL,
  `user_status` enum('pending','active','inactive','deleted') DEFAULT NULL,
  `facebook_connected` varchar(1) DEFAULT NULL,
  `linkedin_connected` varchar(1) DEFAULT NULL,
  `twitter_connected` varchar(1) DEFAULT NULL,
  `email_connected` varchar(1) DEFAULT NULL,
  `last_import` datetime DEFAULT NULL,
  `last_join` datetime DEFAULT NULL,
  `last_commission` datetime DEFAULT NULL,
  `total_imported_contacts` bigint(21) DEFAULT NULL,
  `total_network` bigint(24) DEFAULT NULL,
  `commissions_level_1` double DEFAULT NULL,
  `commissions_level_2` double DEFAULT NULL,
  `commissions_level_3` double DEFAULT NULL,
  `commissions_level_4` double DEFAULT NULL,
  `total_commissions` double DEFAULT NULL,
  `total_store_favorites` bigint(21) DEFAULT NULL,
  `commissions_store` double DEFAULT NULL,
  `last_login` datetime DEFAULT NULL,
  `total_logins` bigint(21) DEFAULT NULL,
  `last_promo_used_on` datetime DEFAULT NULL,
  `last_ticket_on` datetime DEFAULT NULL,
  `total_linked_accounts` bigint(21) DEFAULT NULL,
  `total_linked_institutions` bigint(21) DEFAULT NULL,
  `total_raw_transactions` bigint(21) DEFAULT NULL,
  `total_checkins` bigint(21) DEFAULT NULL,
  `total_perks_used` bigint(21) DEFAULT NULL,
  `total_cashback_used` double DEFAULT NULL,
  `total_reviews` bigint(21) DEFAULT NULL,
  `total_store_views` bigint(21) DEFAULT NULL,
  `total_clicks` bigint(21) DEFAULT NULL,
  `total_searches` bigint(21) DEFAULT NULL,
  `total_locations` bigint(21) DEFAULT NULL,
  `total_ips` bigint(21) DEFAULT NULL,
  `total_devices` bigint(21) DEFAULT NULL,
  `total_open_tickets` bigint(21) DEFAULT NULL,
  `total_closed_tickets` bigint(21) DEFAULT NULL,
  `last_transaction_date` datetime DEFAULT NULL,
  `last_transfer_out_request` datetime DEFAULT NULL,
  `available_balance` double DEFAULT NULL,
  `pending_balance` double DEFAULT NULL,
  `total_withdrawn` double DEFAULT NULL,
  `funds_expiring_in_30_days` double DEFAULT NULL,
  `funds_expired` double DEFAULT NULL,
  `total_withdraw_fees` double DEFAULT NULL,
  `total_cashback_fees` double DEFAULT NULL,
  `total_unmatched_amount` double DEFAULT NULL,
  `total_unmatched` bigint(21) DEFAULT NULL,
  `total_matched_amount` double DEFAULT NULL,
  `total_matched` bigint(21) DEFAULT NULL,
  `pending_transfers_out` double DEFAULT NULL,
  `pending_transfers_in` double DEFAULT NULL,
  `total_financial_alerts` bigint(21) DEFAULT NULL,
  `user_type` varchar(100) DEFAULT NULL,
  `clout_score` double DEFAULT NULL,
  `account_setup_score` double DEFAULT NULL,
  `activity_score` double DEFAULT NULL,
  `referrals_score` double DEFAULT NULL,
  `spending_of_referrals_score` double DEFAULT NULL,
  `spending_score` double DEFAULT NULL,
  `ad_spending_score` double DEFAULT NULL,
  `linked_accounts_score` double DEFAULT NULL,
...
--
-- Структура таблицы `view__user_spending_summary`
--

CREATE TABLE `view__user_spending_summary` (
...
--
-- Структура таблицы `view__where_user_shopped`
--

CREATE TABLE `view__where_user_shopped` (
  `store_id` bigint(20) DEFAULT NULL,
  `user_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `vip_level_categories` (
  `id` bigint(20) NOT NULL,
...
CREATE TABLE `yellowpages_links` (
  `id` bigint(20) NOT NULL,
...
--
-- Индексы таблицы `promotion_default_category_level`
--
ALTER TABLE `promotion_default_category_level`
  ADD PRIMARY KEY (`id`);
...
--
-- Индексы таблицы `store_schedule`
--
...
--
-- AUTO_INCREMENT для таблицы `promotion_default_category_level`
--
ALTER TABLE `promotion_default_category_level`
