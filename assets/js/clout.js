...
// add a remove a class from a field based on the checked status of a given checkbox
function assignClassOnCheck(checkbox, fieldId, className, isReverse)
{
	if(!isReverse){
		if($('#'+checkbox).is(':checked')) $('#'+fieldId).addClass(className);
		else $('#'+fieldId).removeClass(className);
	}
	else {
		if(!$('#'+checkbox).is(':checked')) $('#'+fieldId).addClass(className);
		else $('#'+fieldId).removeClass(className);
...
$(function(){
	$(document).on('keyup', 'input.textfield', function(){
...
// Insert html content before a field with a given ID
function insertBeforeField(fieldId, htmlContent)
{
	$('#'+fieldId).before(htmlContent);
}
...
function refreshUserSession(userId, displayDiv)
{
	$('#time_elapsed').val('0');
	//Refresh the session
	updateFieldLayer(getBaseURL()+'page/refresh_session/u/'+userId,'','',displayDiv,'');
	//Close the fancybox
	$.fancybox.close();
...
// Request phone verification for a user
function requestPhoneVerification()
{
	var seenPhoneVerification = localStorage.getItem('__seen_phone_verification');

	if(!(typeof seenPhoneVerification !== 'undefined' && seenPhoneVerification !== null))
	{
		localStorage.setItem('__seen_phone_verification', 'Y');
		//Add the verification link if it is missing
		if(!$('#__phone_verification_link').length) {
			$("<a href='"+getBaseURL()+"account/verify_telephone' class='shadowbox' id='__phone_verification_link'></a>").prependTo('body');
		}
		$('#__phone_verification_link').click();
	}
}
...
// Add an item to the browser session
function addToBrowserSession(variable, value)
{
...
$(function() {
	...
	$(document).on('focusout','.telephone', function(e){
    	if($(this).val().length < 10) {
			$(this).val('');
			showServerSideFadingMessage('Invalid phone number entered.');
		}
	});

	$('.zipcode').keyup(function(e){
    	if($(this).val().length > 5) $(this).val($(this).val().substr(0,5));
	});
	$('.zipcode').focusout(function(e){
    	if($(this).val().length < 5) {
			$(this).val('');
			showServerSideFadingMessage('Invalid zipcode entered.');
		}
	});
...
// Reposition logo in header
function repositionLogo(){
	if($('.logo').length) $('.logo').offset({left: ($('.navbar-fixed-top').width()/2 - $('.logo').width()/2) });
}
...
function repositionFooter(){
	if($('.bottom-navbar').length){
		var footer = $(".bottom-navbar");
		var lastDiv = footer.prev('div, table');
		if(footer.offset().top < (lastDiv.offset().top + lastDiv.height() + 20)) {
			footer.offset({ top: (lastDiv.offset().top + lastDiv.height() + 20), left: footer.offset().left});
		}
	}
}


// Resize col-span columns to match full span columns
function resizeColSpan(){
	if($('.col-span').length && $('.full-col').length){
		var additionalWidth = $(window).width() < 701? 25: 0;
		$('.col-span').css('width', (($('.full-col').width() + additionalWidth)*100/$('.one-column-table').width())+'%');
	}
}
...
function recenterBlock(){
	if($('.center-block').length && $('.center-block').find('.one-column-table').length == 0){
		var centerHeight = $('.center-block').outerHeight();
		var topOffset = ($(window).height()/2) - (centerHeight/2);
		$('.center-block').offset({top: topOffset });
	}
}


//Resize items in a store
function resizeStoreItems(){
	if($('.store-item').length){
		var containerWidth = $(window).width() - 10;
		var storeWidth = containerWidth/3;
		if(storeWidth < 401) {
			var computedWidth = Math.floor(storeWidth);
			$('.store-item').css('max-width', computedWidth+'px');
		}
		else $('.store-item').css('max-width', '400px');

		// Resize to fit in container if the items are leaving space
		var storeItem = $(document).find('.store-item').first();
		var realWidth = storeItem.outerWidth(true);
		var itemsPerRow = Math.floor(containerWidth/realWidth);

		//Resize the items again if they leave some space at the end
		if(itemsPerRow > 1 && ((realWidth*itemsPerRow) < containerWidth)) {
			var extraWidth = Math.floor((containerWidth - (realWidth*itemsPerRow)) / itemsPerRow);
			$('.store-item').css('max-width', ($(document).find('.store-item').first().width() + extraWidth)+'px');
		}
		else $('.store-item').css('max-width', (containerWidth-20));
	}
}


// Resize the row cell divs to be an equal height
function resizeRowCellDivs(){
	if($('.row-table-cells').length){
		//1. Get tallest div
		var height = 0;
		$('.row-table-cells').children('div').each(function(){
			height = $(this).height() > height? $(this).height(): height;
		});

		//2. Make every div the tallest height
		$('.row-table-cells').children('div').each(function(){
			$(this).height(height);
		});
	}
