...
$(function() {
	...
	$(document).on('touchstart click', '.shadowbox', function(e){
			...
			if($('#__shadowbox').length > 0) $('#__shadowbox').remove();
			...
			$("<div id='__shadowbox' style='position:absolute;display:none;min-height:200px;overflow-y:auto;'><div id='__shadowbox_closer'></div><iframe src='"+url+"' style='width:100%;height:100vh;' marginheight='0' frameborder='0' autoplay='1' id='__shadowbox_iframe' allowfullscreen></iframe></div>").prependTo('body');
			...
			// Put the div and iframe to load the link href
			$("<div id='__shadowbox' style='display:none;'><iframe src='"+url+"' "+closable+" vq=large onload='repositionFrame()' marginheight='0' frameborder='0' id='__shadowbox_iframe' allowfullscreen></iframe><div id='__shadowbox_closer'></div></div>").prependTo('body');
	...
	$(window).resize(function() { 
		if($('#__shadowbox').length){repositionFrame();
			if(isMobile()){
				repositionFrame();
			}
			// Cater for allowing full screen mode
			else if($('#__shadowbox iframe').width() != $(window).width()) {
				$('#__shadowbox').remove();
			}
		}
	});
...
function closeShadowBoxInFrame()
{
	...
	else $('#__shadowbox_closer', window.parent.document).click();
}
