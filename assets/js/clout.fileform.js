...
$(function() {
	...
	if($("#loginform").length > 0)
	{
		$("#loginform").submit(function(e){
	        if($('#loginusername').val().length < 5 || $('#loginpassword').val().length < 5){
	        	e.preventDefault();
	        	showServerSideFadingMessage('All fields are required to continue.');
	        }
	    });
	}
	...
	$(document).on('click focus', '.drop-down-div div', function(e){
		...
		else
		{
			...
			if($(this).html() != 'No Options' && $(this).data('value') != '') {
				...
				else $('#'+fieldId).val($(this).html().replace( /&amp;/g, "&" )).change();
			} else {
			...
			{
				$('#'+fieldId+'__hidden').val($(this).data('value')).trigger("change");
			} else {
	...
	$(document).on('click', '.microform button.submitmicrobtn', function(e){
		...
		inputs.each(function(){
			...
			console.log('FIELD ID: '+$(this).attr('id')+' VALUE: '+$(this).attr('value'));
			...
			{//console.log('FAILED AT: '+$(this).attr('id'));
				activate = false;
				return false;
			}
		});
				...
				error: function( xhr, textStatus, errorThrown) {
      	 		...
      	 		success: function(data) {
					...
					else
					{
						...
						//console.log(formContainer.find('#redirectaction').length);
						...
						else if((resultsDiv === undefined || resultsDiv == '') && formContainer.find('#redirectaction').length > 0){
							//console.log('resultsdiv is empty and redirectaction is not');
							var redirectField = formContainer.find('#redirectaction').first();

							if(data != '') {
								if(data.trim().substring(0, 4) == 'http') document.location.href = data;
						...
						else if(resultsDiv != '' && data != ''){
							//console.log('results div is not null ' + resultsDiv);
						...
						//Simply show the data results in the fading message
							...
							//console.log('resultsdiv is not null ' + resultsDiv);
							showServerSideFadingMessage(data);
						}
	...
	$(document).on('change', '.filefieldurl', function(e){
		...
		if(allowedFormats.indexOf(uploadedFileExtension) != -1 && $(this)[0].files[0].size <= (allowedSize*1024)){
			...
			//Hide fakepath for some browsers when showing the file path
			var reviseFileUrl = uploadedFileUrl.split('/').pop().replace("C:\\fakepath\\","");
			$('#'+parentFieldId).val(reviseFileUrl);
	...
	//Handle cases where a user is entering an URL
	$(document).on('focusout', '.url', function(e){
		if(!validateUrl($(this).val())){
			showServerSideFadingMessage('ERROR: The URL you have provided is invalid.');
			$(this).val('');
		}
	});
	...
	/*
	 * use this class to enable auto validation of a field
	 * by showing a valid field with a .valid class
	 * and an invalid field with a .invalid class
	 *
	 * Example usage:
	 * <input type='text' id='username' name='username'
	 *	class='auto-validate' data-checkurl='account/check_username' data-confirm='isvaliduser' data-minlength='2'
	 *  placeholder='Enter the user name' value='' />
	 */
	$(document).on('keyup', '.auto-validate', function(e){
		var thisField = $(this);
		var actionUrl = getBaseURL()+$(this).data('checkurl');
		var confirmField = $('#'+$(this).data('confirm'));
		...
		if($(this).val().length >= minLength){
			...
			$.ajax({
        		type: "POST",
       			url: actionUrl,
      			data: {check_value: thisField.val()},
      			beforeSend: function() {},
      			success: function(data) {
					if(data == 'VALID'){
						thisField.removeClass('invalid').addClass('valid');
